              ...
              <tbody>
                {data.map(o => (
                  <tr key={o.id} className="border-b border-border/50 hover:bg-surface/50 transition-colors">
                    ...
                    <td className={`py-2 px-3 font-medium ${o.side?.includes('buy') ? 'text-emerald-400' : 'text-red-400'}`}>{o.side?.replace('OrderSide.','').toUpperCase()}</td>
                    ...
                    <td className="py-2 px-3 text-slate-500 text-xs">{o.submitted_at ? new Date(o.submitted_at).toLocaleString() : '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <tbody>
                {data.map(o => {
                  ...
                  return (
                    <tr key={o.id} className="border-b border-border/50 hover:bg-surface/50 transition-colors">
                      <td className="py-2 px-3 text-slate-200 font-medium">{o.symbol}</td>
                      <td className={`py-2 px-3 font-medium ${side === 'buy' ? 'text-emerald-400' : 'text-red-400'}`}>{side?.toUpperCase()}</td>
                      <td className="py-2 px-3 text-slate-300">{o.qty}</td>
                      <td className="py-2 px-3 text-slate-300">{o.filled_qty || '—'}</td>
                      <td className="py-2 px-3 text-slate-300">{o.filled_avg ? `$${Number(o.filled_avg).toFixed(2)}` : '—'}</td>
                      <td className={`py-2 px-3 text-xs font-medium ${STATUS_CSS[status] || 'text-slate-400'}`}>{status}</td>
                      <td className="py-2 px-3 text-slate-500 text-xs">{o.submitted_at ? new Date(o.submitted_at).toLocaleString() : '—'}</td>
                      <td className="py-2 px-3 text-slate-500 text-xs">{o.filled_at ? new Date(o.filled_at).toLocaleString() : '—'}</td>
                    </tr>
                  )
                })}
              </tbody>
